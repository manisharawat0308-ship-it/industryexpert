...
import { AUTO_RISK_SOURCES } from './autoRiskData'
...
export default function AutoRiskAnalysis() {
  const [activeSource, setActiveSource] = useState<string>(AUTO_RISK_SOURCES[0].id)
  ...
  const source: RiskSource = AUTO_RISK_SOURCES.find(s => s.id === activeSource) || AUTO_RISK_SOURCES[0]
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-card border border-gray-100 overflow-hidden">
        <div className="px-6 pt-6 pb-4">
          <h2 className="text-2xl font-black text-navy">Risk Analysis — Automobile Industry</h2>
          <p className="text-sm text-gray-500 mt-1">
            Comprehensive risk assessment across the automotive value chain: vehicle assembly, powertrain manufacturing, supply chain, and testing & validation.
          </p>
        </div>
        ...
        <div>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Natural catastrophe risks — flood, earthquake, cyclone, hailstorm</p>
            </div>
          </div>
        ...
        <div>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Operational risks — fire, explosion, machinery breakdown, EV battery hazards</p>
            </div>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      {source.id === 'assembly' && (
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-6 border border-blue-100">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="text-sm font-bold text-blue-900">EV Transition — New Risk Landscape</h4>
              <p className="text-xs text-blue-700 mt-1 leading-relaxed">
                The ICE-to-EV transition fundamentally changes automotive manufacturing risk. Battery assembly introduces
                Class D fire risks (lithium), toxic gas exposure (HF from LFP/NMC fires), and high-voltage electrical hazards
                absent from traditional auto plants. Existing insurance products, risk engineering standards, and loss history
                are inadequate for gigafactory-scale battery operations. Insurers must develop EV-specific underwriting expertise.
              </p>
            </div>
          </div>
        </div>
      ...
      {source.id === 'supply-chain' && (
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-2xl p-6 border border-amber-100">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="text-sm font-bold text-amber-900">JIT Supply Chain — Amplified BI Exposure</h4>
              <p className="text-xs text-amber-700 mt-1 leading-relaxed">
                Automotive JIT supply chains amplify any supplier incident into massive OEM BI losses. A ₹10 Cr fire at a
                tier-2 supplier can cause ₹500+ Cr BI at the OEM level. Contingent Business Interruption (CBI) cover is
                essential but typically has sub-limits far below actual exposure. Insurers must understand the supply chain
                mapping: which suppliers are single-source, what is the buffer stock, and what is the requalification timeline.
              </p>
            </div>
          </div>
        </div>
